...
class KDBrainHeatPumpEntity(CoordinatorEntity[KDBrainHeatPumpCoordinator]):
    """Base class for entities backed by the heat pump coordinator."""

    _attr_has_entity_name = True

    def __init__(self, coordinator: KDBrainHeatPumpCoordinator, key: str) -> None:
        """Initialise the entity with a stable unique id and device."""
        super().__init__(coordinator)
        assert coordinator.config_entry is not None
        entry_id = coordinator.config_entry.entry_id
